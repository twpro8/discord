...
def chat_model_to_entity(model: ChatOrm) -> Chat:
    return Chat(
        id=model.id,
...
def chat_member_model_to_entity(model: ChatMemberOrm) -> ChatMember:
    return ChatMember(
        id=model.id,
...
def chat_summary_to_entity(row: Row[Any]) -> ChatSummary:
    last_message = (
        LastMessagePreview(
