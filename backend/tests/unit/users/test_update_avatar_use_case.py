...
from tests.unit.users.fakes import FakeCache, FakeStorage, FakeUserRepository, make_user
...
async def test_uploads_avatar_and_updates_url() -> None:
    ...
    use_case = UpdateAvatarUseCase(tx, users, cache, storage)
...
async def test_each_upload_gets_a_new_avatar_url() -> None:
    ...
    storage = FakeStorage(public_base_url="https://files.example.com")
    tx = FakeTransaction()
    use_case = UpdateAvatarUseCase(tx, FakeUserRepository([user]), FakeCache(), storage)
...
async def test_rejects_when_storage_not_configured() -> None:
    user = make_user()
    tx = FakeTransaction()
    use_case = UpdateAvatarUseCase(tx, FakeUserRepository([user]), FakeCache(), None)
