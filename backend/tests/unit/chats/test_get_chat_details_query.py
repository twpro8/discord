...
async def test_returns_summary_for_active_member() -> None:
    ...
    handler = GetChatDetailsQueryHandler(chats, members)
    user_id = uuid4()
    ...
    result = await handler.handle(GetChatDetailsQuery(chat_id=chat.id, user_id=user_id))

    assert result.is_ok
    assert result.value is summary
...
async def test_returns_not_found_for_missing_chat() -> None:
    ...
    result = await handler.handle(GetChatDetailsQuery(chat_id=uuid4(), user_id=uuid4()))
...
async def test_returns_forbidden_for_non_member() -> None:
    ...
    result = await handler.handle(GetChatDetailsQuery(chat_id=chat.id, user_id=uuid4()))
