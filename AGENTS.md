...
modules/<name>/
  ...
  public/
    facade.py            the module's boundary for OTHER modules: a `<Name>Facade` Protocol
                        ...
                        factory. Deliberately framework-free (no FastAPI import) — always a plain
                        function; a facade never owns commit/rollback lifecycle itself (see the
                        Transaction bullet below). Present on modules that at
  ...
  transport/http/
    ...
    dependencies.py      per-module FastAPI DI wiring: a plain provider per repository
                        (`def get_<name>_repository(session: SessionDep) -> XRepository: return
                        XRepositoryImpl(session)`), a provider + `Annotated[X, Depends(get_x)]`
                        Dep alias per use case — taking `TransactionDep` (see the Transaction
                        bullet below) when the use case itself calls `commit()`, or an unused
                        `_tx: TransactionDep` parameter (underscore-prefixed so ruff's `ARG001`
                        doesn't flag it) when the use case never commits but still needs the
                        request's auto-commit to actually run — and, for every other module's
                        facade this one needs, a small locally-defined wrapper
                        (`async def get_x_facade(session: SessionDep) -> XFacade: return
                        build_x_facade(session)`) built only from that producer's
                        `public/facade.py`, never its `transport/`. All provider functions come
                        first, then every `...Dep = Annotated[...]` alias together at the end of
                        the file. Also holds non-use-case, module-owned transport concerns (e.g.
                        auth's cookie extraction). Cross-cutting deps used by several modules
                        (`UserIdDep`, `SessionDep`, `TransactionDep`, `RealtimeNotifierDep`,
                        `RoomMembershipUpdaterDep`, ...) live in `api/v1/dependencies.py` instead,
                        imported from there rather than redefined per module.
```

Key conventions:
- **Domain layer has zero framework/ORM dependencies.** Repository contracts are `Protocol` classes in `domain/repositories/`; only `adapters/` imports SQLAlchemy.
...
- **Repositories are injected directly, and commit is automatic by default.** A use case takes the repository Protocols it needs as plain constructor params — no Unit-of-Work object grouping them. `api/v1/dependencies.py::get_transaction` (`TransactionDep`, `scope="function"`) auto-commits the request's shared `AsyncSession` right before a successful response is sent, and is skipped entirely if the request raised. A use case injects `shared/domain/transaction.py::Transaction` and calls `commit()` itself only when ordering matters: it has work that must run strictly *after* the write is durable (a realtime publish, a cache invalidation, an enqueued job — e.g. `messages.SendChatMessageUseCase`, `users.UpdateUserUseCase`), or it must commit before raising, since a raised exception skips auto-commit too (`auth.token_helper.get_valid_refresh_token`'s revoke-then-raise branch). A use case whose own write has no such ordering requirement (e.g. `chats.UpdateChatUseCase`) takes no `Transaction` at all and relies on auto-commit — but its DI provider still takes an unused `_tx: TransactionDep` to force that dependency to actually build, since otherwise nothing in the request would reference it. Code that runs entirely outside the FastAPI request lifecycle (a Celery task self-composing its session, see `email.transport.tasks.send_email_task`) has no auto-commit at all and must commit explicitly at every branch.
- Cross-module reads sometimes reach directly into another module's ORM models (e.g. `chats` repository joins `messages` and `users` ORM models) rather than going through that module's repository — this is an accepted shortcut for read-heavy queries, not a pattern to generalize into write paths. A use case that needs another module's *write* behavior as part of its own atomic operation (e.g. `servers.CreateServerUseCase` creating a server's default channel, `messages.SendChatMessageUseCase` incrementing a chat's sequence counter) still holds a same-session repository/use-case instance built by its own `transport/http/dependencies.py` — via the producer module's facade where one exists (`ChannelsFacade.create_default_channel`), or a directly-injected repository where the write is cheap/narrow enough not to need one (`messages`' use cases taking `chats`'/`channels`' repositories directly) — never a second, independent session/transaction, since that wouldn't share the caller's uncommitted work.
...
- `api/v1/router.py` is the composition root: a new module's HTTP router is imported directly and added to its `_MODULE_ROUTERS` list, which `build_api_v1_router()` mounts under `/api/v1`. There's no per-module wiring-registration file — no `module.py`, no `composition/` package (the old `composition.py`/mediator-registration step is gone; each module's own `transport/http/dependencies.py` is self-contained). `messages` has no top-level router of its own — its two routers (`chat_message_router`, `channel_message_router`) are imported directly by `chats`' and `channels`' routers, since its HTTP surface is a sub-resource of theirs.
...
- Unit tests (`tests/unit/<module>/`) construct use cases directly with hand-written in-memory fakes for that module's repository Protocols and any facade Protocols it depends on (colocated as `tests/unit/<module>/fakes.py`, not centralized) — no DB, no HTTP, no app instance. Call the use case with keyword arguments (`await use_case(chat_id=..., user_id=...)`) and assert on the returned value, or wrap the call in `pytest.raises(SomeLumiereError)` for failure cases — no `Result`/`.is_ok` assertions. A shared `tests/unit/fakes.py::FakeTransaction` stands in for `Transaction` wherever a use case takes one — no per-module fake needed, since `Transaction` is a `Protocol`. A use case that takes another module's repository directly for a same-transaction write (e.g. `messages`' use cases taking `chats`'/`channels`' repos, needed for `increment_sequence`) reuses that other module's already-built fakes rather than duplicating them; a `Fake<Name>Facade` (e.g. `FakeChatsFacade`, `FakeUsersFacade`) is likewise colocated in the producer module's own `tests/unit/<module>/fakes.py` and imported by consumers.
