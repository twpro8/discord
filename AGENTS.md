...
modules/<name>/
  ...
  composition.py        register_<name>_handlers(registry: HandlerRegistry) — registers one
                        ...
                        at app startup, from `composition/container.py::build_container()` — the
                        same composition-root function that also wires up this module's HTTP
                        router — and not per-request. Also where a factory builds *other*
...
- `composition/container.py::build_container()` is the composition root — the single explicit place the whole application gets wired together, called once from `main.py::create_app()` and stored on `app.state.container`. Its `Container` dataclass holds both `module_routers: list[APIRouter]` (each module's `register_<name>_module()` appended, then mounted under `/api/v1` by `api/v1/router.py`) and `handler_registry: HandlerRegistry` (each module's `register_<name>_handlers(registry)` called into it — see the `composition.py` row above). A new module needs a `module.py` (router) and a `composition.py` (handler factories) *and* a call to both `register_<name>_module()` and `register_<name>_handlers()` added inside `build_container()` to actually be wired up.
...
- **Singleton / process lifetime** — built once in `main.py`'s `lifespan()`/`create_app()`, stored on `app.state`: the DB engine and sessionmaker (`app.state.db_engine`/`.session_factory`, disposed on shutdown alongside Redis), the Redis client (`app.state.redis`), the read-model cache and event bus (`app.state.cache`/`.event_bus`), the composition root `Container` (`app.state.container` — its `handler_registry` field is what `get_mediator` reads, its `module_routers` field is what gets mounted), and `Settings`. None of these are per-request work, and none use `@lru_cache`-decorated module functions as a substitute for this — that pattern was replaced (the DB engine/sessionmaker used to be `@lru_cache`d in `core/database/session.py`, with no disposal hook, before being moved here for consistency with everything else).
